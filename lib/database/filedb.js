...
class FileDB extends Database {
  ...
  parseCategories() {
    try {
      var categoryFile = load(fs.readFileSync(this.path + "/categories.yml").toString());
  ...
  async updateGlobals() {
    ...
    try {
      ...
      for(i in this.dbInfo.categoryList) {
        ...
        questionPool = load(fs.readFileSync(this.path + `/questions_${this.dbInfo.categoryList[i].id}.yml`).toString());
        ...
        for(var i2 in questionPool.questions) {
          ...
          // Append the category so we don't need some form of witchcraft to re-acquire it.
  ...
  async fetchQuestions(options) {
    ...
    for(var i = 0; i <= options.amount-1; i++) {
      ...
      try {
        var exclusionPool;
        if(!isRandomCategory) {
          exclusionPool = load(fs.readFileSync(this.path + `/questions_${category}.yml`).toString()).questions;
        }
        else {
        ...
        if(typeof token !== "undefined") {
          if(isRandomCategory) {
            ...
            // Initialize this category if necessary.
