...
class TriviaDiscord extends TriviaInstance {
   ...
   * @param {Function} reply - Callback for replying to the command within the channel.
   * @param {Function} replyDirect - Callback for replying to the command as a private or ephemeral message. In slash command mode, this will use an ephemeral message. For text commands, it will be a DM to the user if available.
   ...
   * @returns {void}
   */
  doCommand(cmd, reply, replyDirect, channel, user, member, guild, msg, args) {
    ...
    if(cmd.startsWith("PLAY ADVANCED") || (this.getConfig("use-slash-commands") && cmd === "SETUP")) {
      ...
      commands.triviaPlayAdvanced(reply, replyDirect, void 0, channel.id, channel, user, cmd.replace("PLAY ADVANCED",""), undefined, undefined, args);
      return;
    }
    ...
    if(cmd.startsWith("PLAY HANGMAN ") || cmd === "PLAY HANGMAN" || (msg === null && cmd === "HANGMAN")) {
      ...
      commands.triviaPlay(reply, replyDirect, channel.id, user.id, guild, categoryInput, "hangman");
    ...
    if(cmd.startsWith("PLAY ") || cmd === "PLAY") {
      ...
      commands.triviaPlay(reply, replyDirect, channel.id, user.id, guild, categoryInput, mode);
      return;
    }
  ...
  parseSlash(commandName, interaction) {
    ...
    if(!this.getConfig("use-slash-commands", interaction.channel)) {
      ...
      console.warn("Warning: Got slash command attempt while commands are disabled. Command will be ignored with a message to the user. See the link below for more information.\nhttps://lakeys.net/triviabot/install#commonErrors-userRanSlash");
      interaction.reply({
        content: `This bot is using text commands instead of slash commands. Try \`${prefix}help\`, etc. instead.`,
        ephemeral: true
      });
    ...
    const reply = async (response) => {
      ...
      if(!interactionReplySent && !interaction.replied) {
        interactionReplySent = true;
        try {
          const res = await interaction.reply({...msg, fetchReply: true});
  ...
  parseText(str, msg) {
    ...
    if(str.startsWith(prefix)) {
      ...
      this.doCommand(cmd, reply, replyDirect, msg.channel, msg.author, msg.member ?? undefined, msg.guild ?? undefined, msg);
    }
  }
  ...
  //    msg: Message Object
  //    neverDelete: If enabled, message will not auto-delete even if configured to
  // TODO rewrite
  ...
  buttonPress(interaction, answer, userId, username) {
    ...
    if(game.getConfig("disallow-answer-changes", interaction.channel.id) && typeof game.usersActive[userId] !== "undefined") {
      const msg = interaction.reply({content: "You have already answered this question.", ephemeral: true});
      this.emit("message_sent", msg, {ephemeral: true});
      return;
    }
